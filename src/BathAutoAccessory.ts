...
import { PLATFORM_NAME, BATH_AUTO_FIXED_ID } from './settings.js'
import { Switch } from './types.js'

const ECHONET_LITE_DEVICE_ID = '027201'
const POWER_STATE_EPC = 0xe3
...
export class BathAutoAccessory {
  ...
  async init() {
    const name = `${this.configs.name} Bath Auto`
    const uuid = this.platform.api.hap.uuid.generate(BATH_AUTO_FIXED_ID)
    ...
    subscribe(this.configs.ip, ECHONET_LITE_DEVICE_ID, (els) => {
      ...
      for (const key in DETAILs) {
        ...
        if (!value || value.length === 0) {
          ...
          this.state.switch =
            value === POWER_STATE_ON.toString(16)
              ? Switch.ACTIVE
              : Switch.INACTIVE
